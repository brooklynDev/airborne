...
describe 'expect_json with path' do
  ...
  it 'should ensure at least one match' do
    mock_get('array_with_nested')
    get '/array_with_nested'
    expect { expect_json('cars.?.owners.?', name: 'Bart Simpsons') }.to raise_error(RSpec::Expectations::ExpectationNotMetError)
  end
  ...
  it 'should ensure one match that matches all with lambda' do
    mock_get('array_with_nested')
    get '/array_with_nested'
    expect { expect_json('cars.?.owners.*', name: ->(name) { expect(name).to eq('Bart Simpsons') }) }.to raise_error(RSpec::Expectations::ExpectationNotMetError)
  end
